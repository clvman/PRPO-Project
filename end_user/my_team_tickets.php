...
$row_owner_teams = mysqli_fetch_assoc($query__owner_teams);
 $owners_team_name= $row_owner_teams['team'];
 echo $owners_team_name;
...
responsive: true,
    ...
    action: function (e, dt, node, config)
    {
        ...
        
        {
			...
			serverSide: true,
			ajax: {
				url: 'datatable_api/server_my_teams_tickets.php?user='+"<?php echo $_SESSION['NCE_TT_end_user'] ?>",
				type: 'POST',
				data: {
...
{   data: 'status',
    render:function (status) {
      if(status=='Resolved'){return '<span class="badge badge-success">'+status+'</span>'}
      	                                      //
           else{return status }

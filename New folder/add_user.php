...
if (isset($_POST['username'])){
 ...
 $query_0 = "SELECT * FROM `users` WHERE (username='$username'and email='$email' and  role='$role')";
 $result_0 = mysqli_query($con,$query_0) or die(mysql_error());
  if(mysqli_num_rows($result_0) > 0){
        ...
        $query = "INSERT into `users` (username, password,role,email)VALUES ('$username', '".md5($password)."','$role','$email')";
        $result = mysqli_query($con,$query);
        if($result){
            
 echo "<script>alert('User registered successfully');window.location = 'add_user.php';</script>" ;
